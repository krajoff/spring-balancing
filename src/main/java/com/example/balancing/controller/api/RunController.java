...
public class RunController {
    ...
    @PostMapping
    public ResponseEntity<RunDto> create(@PathVariable UUID stationId, @PathVariable UUID unitId, @RequestBody RunDto dto) {
        log.info("Create run for unit {} in station {}", unitId, stationId);
        return ResponseEntity.ok(runService.create(unitId, dto));
    }

    @PutMapping
    public ResponseEntity<RunDto> update(@PathVariable UUID stationId, @PathVariable UUID unitId, @RequestBody RunDto dto) {
    ...
    @DeleteMapping
    public ResponseEntity<Void> delete(@PathVariable UUID stationId, @PathVariable UUID unitId, @RequestBody RunDto dto) {
        if (unitId == null || dto.getId() == null) throw new IllegalArgumentException(EntityTypeException.RUN);
